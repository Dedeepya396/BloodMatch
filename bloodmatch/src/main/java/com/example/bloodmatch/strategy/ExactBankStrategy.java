...
// Exact bank strategy: only consider packets with requested blood group
public class ExactBankStrategy implements BankMatchingStrategy {
    ...
    public List<MatchResponse> matchBanks(List<BloodBank> allBanks, BloodRequest request, boolean applyDistanceFilter, BloodBankService bloodBankService) {
        ...
        return allBanks.stream()
                .filter(bb -> {
                    ...
                    return packets.stream().anyMatch(p -> "AVAILABLE".equals(p.getStatus()) && p.getUnits() > 0);
                })
                .map(bb -> {
