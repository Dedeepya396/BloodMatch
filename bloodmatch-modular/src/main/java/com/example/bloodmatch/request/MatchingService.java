...
import com.example.bloodmatch.notification.DonorNotifier;
...
import com.example.bloodmatch.core.CompatibilityFirstStrategy;
import com.example.bloodmatch.core.MatchingStrategy;
import com.example.bloodmatch.core.SmartMatchingStrategy;
    ...
    public List<MatchResponse> findMatches(BloodRequest request) {
        ...
        if ("HIGH".equalsIgnoreCase(request.getUrgency())) {
            ...
            bankMatches = allBanks.stream()
                    .filter(bb -> {
                        double dist = DistanceUtil.calculate(
        ...
        if ("HIGH".equalsIgnoreCase(request.getUrgency())) {
            ...
            donors = donors.stream()
                    .filter(d -> DistanceUtil.calculate(
